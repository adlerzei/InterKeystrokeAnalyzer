...
file_handler.make_read_path_and_file(file_name, "4810", "1", "4a")
    ...
    for task_id in range(1, 2):
        if utils.is_task_completed(user_id, task_id, file_handler):
            for (s1, s2) in char_pairs:
                ...
                file_handler.make_read_path_and_file(file_name, user_id, task_id, string_to_enter)
...
#    file_handler.make_write_path_and_file(config.transitions_file_name, user_id)
#    analyzer.save_transition_probabilities(file_handler)

#    file_handler.make_write_path_and_file(config.observations_file_name, user_id)
